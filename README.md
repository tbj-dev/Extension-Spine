...
Setup
- Copy `pixi.min.js` and `pixi-spine.min.js` into `lib/` (or switch to CDN in `index.js`).
- In ST, install this extension folder (third-party).
- Open Settings → Extensions → Spine Viewer (MVP), toggle Enabled.
- In the browser console, set a mapping (.json or .skel is supported), e.g.:
